...
import { notifyLicenseUsed, notifySuspiciousActivity } from '../webhook.js';
...
// Ningún jugador real hace más de esto en una hora consultando posiciones
// una por una a mano. Configurable por variable de entorno por si hace
// falta ajustarlo sin tocar código.
const HOURLY_LIMIT = Number(process.env.LICENSE_HOURLY_LIMIT) || 30;
...
router.post('/validateLicencia', async (req, res) => {
    ...
    const { licencia, archimonsterId, turnstileToken } = req.body;
    ...
    if (!turnstile.ok) {
        return res.status(403).json({ message: 'Verificación anti-bot fallida. Recarga la página e inténtalo de nuevo.' });
    }
    ...
    try {
        ...
        // Límite de volumen total por hora: el cooldown de arriba solo evita
        // ráfagas rápidas, pero un script "paciente" que respeta el cooldown
        // podría igual hacer cientos de consultas seguidas. Esto lo detecta.
        if (!licenciaExistente.usageWindowStart || now - licenciaExistente.usageWindowStart > HOUR_MS) {
            licenciaExistente.usageWindowStart = now;
            licenciaExistente.usageWindowCount = 0;
            licenciaExistente.usageAlerted = false;
        }
        licenciaExistente.usageWindowCount += 1;

        if (licenciaExistente.usageWindowCount > HOURLY_LIMIT) {
            if (!licenciaExistente.usageAlerted) {
                licenciaExistente.usageAlerted = true;
                notifySuspiciousActivity({
                    pcId: licenciaExistente.pc_id,
                    licencia: licenciaExistente.licencia,
                    count: licenciaExistente.usageWindowCount,
                    limit: HOURLY_LIMIT
                });
            }
            await licenciaExistente.save();
            return res.status(429).json({
                message: 'Has alcanzado el límite de consultas por hora para esta licencia.'
            });
        }
